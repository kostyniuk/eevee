...
- **PR-close harvest job**: on PR `closed`, one pass performs the reaction harvest and builds the Eval Comparison pair (skipped if the head SHA equals the last-reviewed SHA). Each side contains mapped excerpts for every stored Finding, with overlapping excerpts collapsed. When a mapped final line is absent from the final diff, its source context is fetched at the merged SHA; long comparisons span multiple Slack blocks without dropping Findings. If any Finding cannot be mapped safely, no partial pair is published. The job then hands the pair to the DAO and notification service.
