...
- **PR-close harvest job**: on PR `closed`, one pass performs the reaction harvest and builds the Eval Comparison pair (skipped if the head SHA equals the last-reviewed SHA), then hands both to the DAO and the notification service.
